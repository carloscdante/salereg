        ...
        for (let i = 1; i < 6; i++) {
            Seller.find({rank: i}, function(err, allSellers){
                allSellers.forEach(seller => {
                    ...
                    `- ${chalk.bold(chalk.white(seller.salesTotal))}`)
                })
            })   
        }
    })
...
function list(){
        ...
        Seller.find({}, function(err, seller){
            if(err){
                ...
                updateRanks()
                //The for loop is for ranking the list from highest total of sales to lowest, finding each seller and listing the sales based on the seller's rank.
                //e.g. If the seller is in fourth place, his sales will appear right after the sales of the third place end.
                for (let i = 1; i < 6; i++) {
                    Seller.find({rank: i}, function(err, allSellers){
                        allSellers.forEach(seller => {
...
function addSale(date, seller, customer, product, price){
    Seller.find({name: seller}, function(err, name){
        if(err){
            ...
            //If there is, add the sale to the database, sum the value of the sale to the seller's total sales and update the ranks.
                ...
                if(err){
                    ...
                    if(err){
                        ...
                        Seller.find({name: seller}, function(err, seller){
                            ...
                            //This section sums the price of the sale to the salesTotal parameter of the desired seller.
                    ...
                    //If a sale with the same unique code generated is found, generate another code and try to run the function again with the new one.
                    uniqueCode = getRandomInt(10000, 50000)
                        ...
                        if(err){
                            ...
                            Seller.find({name: sale[0].sellerName}, function(err, seller){
                                if(err){
                                    ...
                                    //This code block transfers the value of the sale from the original seller to the seller in the given parameter.
                                    let totalSales = seller[0].salesTotal
